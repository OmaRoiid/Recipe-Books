...
export class DataServiceStorage {
  ...
  storeLapTops() {
    ...
    return this.mHttpClient.put(
      'https://techbooks-838de-default-rtdb.firebaseio.com/laptops.json',
      lapTops,
      {
        observe: 'response',
      }
      ...
      .pipe(
        map((laptops) => {
          return laptops.map((laptop) => {
            // cus ingredients is  option in the  add form and if it's empty
            // you should insure it will work will and not make a bug in the structure of the json file in firebase
            return {
              ...laptop,
              ingredients: laptop.ingredients ? laptop.ingredients : [],
            }; // if it's empty set an empty arr filed in the  file jseon
          });
        }),
        tap((res) => {
